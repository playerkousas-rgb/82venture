...
export const SHEET_TABS = ['帳目', '物資', '團員', '收支申報', '通告', '報名', '會議', '設定'];
...
function syncAll(body) {
  ...
  counts['members'] = writeTab(ss, body, '團員', tables.members, schema.members, ['name', 'eng', 'birthday', 'role', 'status', 'phone', 'email', 'join', 'note']);
...
function writeSignups(ss, body, notices) {
  ...
  notices.forEach(function (n) {
    (n.signups || []).forEach(function (s) {
      var v = s.values || {};
      rows.push([body.unit, n.id, n.title && n.title.zh || '', s.at || '', v.name || '', v.contact || '', JSON.stringify(v)]);
    });
  });
  var name = sheetName('報名', body.unit);
  var sh = ss.getSheetByName(name) || ss.insertSheet(name);
  sh.clear();
  var header = ['旅團', '通告編號', '通告標題', '報名時間', '姓名', '聯絡', '全部欄位(JSON)'];
  sh.getRange(1, 1, 1, header.length).setValues([header]).setFontWeight('bold');
...
function appendSignup(body) {
  ...
  var sh = ss.getSheetByName(name) || ss.insertSheet(name);
  if (sh.getLastRow() === 0) {
    sh.appendRow(['旅團', '通告編號', '通告標題', '報名時間', '姓名', '聯絡', '全部欄位(JSON)']).getRange(1, 1, 1, 7).setFontWeight('bold');
  }
  sh.appendRow([body.unit || '', p.noticeId || '', p.noticeTitle || '', p.at || new Date(), v.name || '', v.contact || '', JSON.stringify(v)]);
}
